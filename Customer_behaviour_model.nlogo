patches-own [ section-name item-id stock expiry-date ];BHARGAVRAM ; Item Id is the property of the patch, means that each patch has its own unique ID
...
to divide-into-sections ;Function to divide the store into different sections by giving the details and coordinates of each section
  ...
  ask patches with [ pcolor = 85 ] [
    ...
    set expiry-date random expire-prob  ; BHARGAV RAM
    set stock random 30
  ]
...
to remove-item-from-list ; As stated above, this function will remove the item from the list in two cases.
  ...
  ifelse item-stock = 0 or
  item-expiry-date =  0 or
  purchased-item-price > money-remaining [ ; Case 1: If the customer' possesion money is less than the item's price, then the item will be removed from the items list and will be added in the skipped cart
...
* They select the product which is nearer to them and validate it for the expiry date.
...
* Before starting the checkout process, customers revisits their shopping list whether all the items are covered or, are there any items that they forgot to visit. If they miss to visit some items, they again return into the store, pick the items and again move to register
